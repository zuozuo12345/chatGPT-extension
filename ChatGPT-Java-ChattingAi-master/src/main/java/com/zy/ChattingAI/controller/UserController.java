...
import com.zy.ChattingAI.api.mengwang.SmsComponent;
import com.zy.ChattingAI.dto.*;
...
public class UserController {
    ...
    public ReturnResult sendCode(@Valid @RequestBody SendCodeRequest request) throws Exception {
        ...
        // 发送验证码
        return smsComponent.send(request.getPhone(), code).getSuccess() ? ReturnResult.ok()
                : ReturnResult.error().message("发送验证码错误");
    }
    ...
    // public ReturnResult register(@Valid @RequestBody UserRegisterRequest req,
    ...
    //
    // // 删除验证码
    // stringRedisTemplate.delete(String.format(RedisKeys.USER_REGISTER_CODE
    // ,req.getPhone()));
    //
    // //验证码通过，真正注册
    ...
    //
    // // 给新用户创建第一个聊天会话
    // userSessionService.save(register.getId(), register.getUsername() + "的聊天室",
    // SessionType.NORMAL_CHAT);
    //
    // // 给新用户直接登录
    ...
    public ReturnResult getUserListByCondition(@RequestBody UserListRequest userListRequset,
            @PathVariable("limit") Long limit, @PathVariable("page") Long page) {
