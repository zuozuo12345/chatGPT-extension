...
import com.zy.ChattingAI.api.openai.resp.Embedding;
import com.zy.ChattingAI.api.openai.resp.EmbeddingResp;
import com.zy.ChattingAI.api.pinecone.PineconeApi;
import com.zy.ChattingAI.api.pinecone.req.PineconeDeleteReq;
import com.zy.ChattingAI.api.pinecone.req.PineconeInsertReq;
import com.zy.ChattingAI.api.pinecone.req.PineconeQueryReq;
import com.zy.ChattingAI.api.pinecone.req.PineconeVectorsReq;
import com.zy.ChattingAI.api.pinecone.resp.PineconeQueryResp;
import com.zy.ChattingAI.entity.DataSqlEntity;
...
import com.zy.ChattingAI.service.*;
import com.zy.ChattingAI.sse.OpenAISessionChatSSEListener;
import com.zy.ChattingAI.utils.MilvusClientUtil;
import com.zy.ChattingAI.utils.RecursiveCharacterTextSplitter;
import com.zy.ChattingAI.utils.ResultCode;
import com.zy.ChattingAI.utils.VerificationCodeGenerator;
...
public class FileChatServiceImpl implements FileChatService {
    ...
    private LoadingCache<Integer, Deque<SessionChatRecordEntity>> windowRecordCache = Caffeine.newBuilder()
    ...
    @Override
    public String uploadFile(MultipartFile multipartFile, String userId, String apiKey, ChatGPTReq chatGPTReq,
            boolean usePinecone) {
        ...
        if (usePinecone) {
            ...
            PineconeInsertReq pineconeIndexReq = PineconeInsertReq.builder().vectors(vectors).namespace(collectionName)
                    .build();
            result = PineconeApi.insertEmbedding(pineconeIndexReq,
                    adminApiKeyService.roundRobinGetByType(ApiType.PINECONE));
        ...
        String summaryMessage = getContext(collectionName, promptService.getByTopic(Prompt.PDF_SUMMARY_TEMPLATE.topic),
                apiKey, null, usePinecone);
        ...
        SessionChatRecordEntity replyRecord = new SessionChatRecordEntity(
                userSessionEntity.getSessionId(), Role.ASSISTANT.name, resp.getMessage(),
                resp.getUsage().getCompletion_tokens());
     ...
     */
    @Override
    public ChatGPTResp chatWithFile(String userId, Integer sessionId, String message, String apiKey,
            ChatGPTReq chatGPTReq, boolean usePinecone) {
    ...
    // 文件流式对话
    @Override
    public void streamChatWithFile(String userId, Integer sessionId, String message, String apiKey,
            ChatGPTReq chatGPTReq, SseEmitter sseEmitter, boolean usePinecone) {
    ...
    private String getStandaloneQuestion(String message, Deque<SessionChatRecordEntity> windowRecords,
            Integer sessionId, String apiKey, String userId) {
        ...
        while (!windowRecords.isEmpty()
                && windowRecordsTokens + askTokenNum + qaPromptNum + (windowRecords.size() + 1) * 2 > K) {
     ...
     * @return
     */
    private String getContext(String collectionName, String message, String apiKey,
            Deque<SessionChatRecordEntity> windowRecords, boolean usePinecone) {
        /* ↓检索文档 */
        ...
        if (usePinecone) {
            ...
            PineconeQueryReq pineconeQueryReq = PineconeQueryReq.builder().namespace(collectionName).topK(TOP_K)
                    .includeMetadata(true).vector(qaEmbed).build();
            PineconeQueryResp pineconeQueryResp = PineconeApi.queryEmbedding(pineconeQueryReq,
                    adminApiKeyService.getBestByType(ApiType.PINECONE));
            ...
            List<List<Float>> qaEmbed = embed.getData().stream().map(Embedding::getEmbedding)
                    .collect(Collectors.toList());
        ...
        int contextRecordsTokens = ChatGPTApi.getTokenNum(promptService.getByTopic(Prompt.FINAL_PROMPT_TEMPLATE.topic))
                +
        ...
        return String.format(promptService.getByTopic(Prompt.FINAL_PROMPT_TEMPLATE.topic), context.toString(),
                chatHistory.toString(), message);
    }
    ...
    public boolean dropCollection(String userId, Integer sessionId, boolean usePinecone) {
        String collectionName = "id_" + userId + "_" + sessionId;
        if (usePinecone) {
            PineconeDeleteReq pineconeDeleteReq = PineconeDeleteReq.builder().deleteAll(true).namespace(collectionName)
                    .build();
    ...
    private DataSqlEntity docToEmbed(String doc, String apiKey, boolean usePinecone) {
        ...
        List<List<Float>> ll = embeddingResp.getData().stream().map(Embedding::getEmbedding)
                .collect(Collectors.toList());
