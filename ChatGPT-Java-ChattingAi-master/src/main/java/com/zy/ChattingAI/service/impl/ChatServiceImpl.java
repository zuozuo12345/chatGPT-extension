...
import com.zy.ChattingAI.sse.OpenAIOneShotChatSSEListener;
...
public class ChatServiceImpl implements ChatService {
    ...
    private LoadingCache<Integer, Deque<SessionChatRecordEntity>> gameWindowRecordCache = Caffeine.newBuilder()
            .initialCapacity(1024)
            // 手动设置10分钟缓存过期，
            .expireAfterAccess(10L, TimeUnit.MINUTES)
            // 缓存填充策略：同步加载。缓存中有则用，没有则调用loadWindowRecordCache获得value并存入缓存中
     ...
     * @param sessionId
     */
    private Deque<SessionChatRecordEntity> getWindowRecordsBySessionTypeAndId(SessionType sessionType,
            Integer sessionId) {
        switch (sessionType) {
            case NORMAL_CHAT:
