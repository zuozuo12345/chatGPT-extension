package com.zy.ChattingAI.api.openai;
...
import com.zy.ChattingAI.api.openai.req.EmbeddingReq;
...
import com.zy.ChattingAI.global.constant.GlobalConstant;
import com.zy.ChattingAI.utils.DateTimeFormatterUtil;
import com.zy.ChattingAI.utils.OkHttpClientUtil;
...
public class ChatGPTApi {
    ...
    public static ChatGPTResp sessionReq(ChatGPTReq gpt, String apiKey) {
        ...
        Request request = new Request.Builder()
                ...
                .post(RequestBody.create(MediaType.parse(ContentType.JSON.getValue()),
                        JSONUtil.parseObj(gpt).toString()))
    ...
    public static EmbeddingResp embeddings(List<String> input, String apiKey) {
        ...
        Request request = new Request.Builder()
                ...
                .post(RequestBody.create(MediaType.parse(ContentType.JSON.getValue()),
                        JSONUtil.parseObj(embeddingReq).toString()))
                .header(AUTHORIZATION_STR, "Bearer " + apiKey)
                .build();
        Response response = null;
        try {
        ...
        try {
            ...
            if (accessUntil * GlobalConstant.TEN_K < System.currentTimeMillis()) {
                log.warn("检查到apiKey：{}过期，过期时间{}", apiKey,
                        Instant.ofEpochMilli(accessUntil * GlobalConstant.TEN_K).atZone(ZoneId.systemDefault())
                                .toLocalDate());
                // 不抛异常，因为特殊的apiKey过期了还能使用
                // throw new BaseException(OpenAiRespError.OPENAI_APIKEY_EXPIRED.code,
                // OpenAiRespError.OPENAI_APIKEY_EXPIRED.msg);
            }

            // 获取总额度
            BigDecimal totalAmount = BigDecimal
                    .valueOf(Double.parseDouble(String.valueOf(subMap.get("hard_limit_usd"))));
            ...
            while (true) {
                ...
                BigDecimal curUsage = BigDecimal
                        .valueOf(Double.parseDouble(String.valueOf(usageMap.get("total_usage"))));
