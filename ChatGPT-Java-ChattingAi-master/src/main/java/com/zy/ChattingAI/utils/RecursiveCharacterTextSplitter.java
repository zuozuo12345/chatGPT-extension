    ...
    private List<String> mergeSplits(List<String> splits, String separator) {
        ...
        for (String d : splits) {
            ...
            if (total + len + (separatorLen > 0 && !currentDoc.isEmpty() ? separatorLen : 0) > chunkSize) {
                if (total > chunkSize) {
                    System.out.println("Warning: Created a chunk of size " + total
                ...
                if (!currentDoc.isEmpty()) {
                    ...
                    while (total > chunkOverlap
                            || (total + len + (separatorLen > 0 && !currentDoc.isEmpty() ? separatorLen : 0) > chunkSize
                                    && total > 0)) {
                        total -= currentDoc.get(0).length()
                                + (separatorLen > 0 && currentDoc.size() > 1 ? separatorLen : 0);
                        currentDoc.remove(0);
                    }
                }
            }
